...
1. `controller.rs` 在候选路径查找官方 `Grok.exe`：
   - `%LOCALAPPDATA%\Programs\@grokdesktop\Grok.exe`
...
- 不连接任意调试端口。
- 不接受非回环 WebSocket。
- 不向非 Grok 主界面 target 注入。
- 不按进程名粗暴结束所有 `Grok.exe`；必须比较官方安装路径的完整路径。
- 主进程 Inspector 只用于启动瞬间补丁，完成后必须关掉；失败路径也要 `resume`，
  不能把 Grok 留在 `--inspect-brk` 暂停态。
...
```text
window.__GROK_BACKGROUND_STUDIO__
```

主要对象：

- `#grok-background-style`：普通 DOM 样式。
- `#grok-background-layer`：固定背景媒体层。
- `#grok-background-media`：图片或视频。
- `#grok-background-tile`：平铺模式。
- `#grok-background-overlay`：整页底色。
...
Grok 渲染页不能稳定读取 Studio 回环 HTTP 媒体 URL。Rust 后端读取受管媒体、
...
```text
%LOCALAPPDATA%/GrokBackgroundStudio
```
...
- 协议：`pluginProtocol: 1`
- 启动：`Grok Background Studio.exe --plugin`
- Pipe：`\\.\pipe\background-studio-grok`
- `pluginId`：`grok`
- Release 资产：`GrokBackgroundStudio-<version>-plugin.zip`
...
- 再解压/复制到 `%LOCALAPPDATA%\BackgroundStudio\plugins\grok\<version>\`
- 同步改 `%LOCALAPPDATA%\BackgroundStudio\plugins.json` 的 `installedVersion`
- 替换前先停掉占用中的插件进程

正式环境优先在壳里对 Grok 点「检查更新 / 安装」。

## 已解决的典型故障

### 标题栏白条 / 黑条 / 抖动 / 按钮消失

原因：独立 Win32 overlay 与 Grok 窗口不同步。
...
### 应用后「没有可用的 Grok CDP 运行时」

原因：启动插件时 Grok 还没带着调试端口在跑，或 runtime.json 失效。
