...
Grok 更新后 DOM 可能变；改样式前必须再用 CDP 核对，不要只靠截图猜。

## 目标 page

- 主界面：`file://.../Programs/@grokdesktop/.../out/renderer/index.html`
  （或 `Programs/Grok/...`）
- 可选：`https://grok.ai` / `https://*.grok.ai`（若出现独立 webview，需单独确认是否注入）

只向 Grok 主应用 page 注入；辅助/空白 target 不要乱注入。
...
- 主题：`html.light` / `html.dark`（探测后设 `grok-background-dark`）
- 状态：`window.__GROK_BACKGROUND_STUDIO__`
...
| 底色遮罩 | overlay 设置 | `#grok-background-overlay` | 直接 opacity |
| 媒体 | `--cbg-opacity` × 路由强度 | `#grok-background-layer` | — |
