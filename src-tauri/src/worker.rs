...
pub struct WorkerState {
    controller: Arc<Mutex<GrokController>>,
...
impl WorkerState {
    ...
    pub fn load_from(data_directory: PathBuf) -> Result<Self, String> {
        ...
        let controller = GrokController::load(&data_directory);
...
mod tests {
    ...
    fn temp_state() -> WorkerState {
        let dir = std::env::temp_dir().join(format!("grok-worker-{}", Uuid::new_v4()));
