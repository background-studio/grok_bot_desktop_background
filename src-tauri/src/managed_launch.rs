...
pub const MSG_WAITING: &str = "已启用，等待 Grok 启动";
pub const MSG_EXISTING: &str = "Grok 已在运行，点立即接管可重启";
pub const MSG_TAKING_OVER: &str = "正在接管 Grok";
pub const MSG_AUTO_APPLIED: &str = "背景已自动应用";
pub const MSG_SUSPENDED: &str = "托管已暂停";
pub const MSG_UNCONFIGURED: &str = "尚未配置背景";
pub const MSG_DEBUG_TIMEOUT: &str = "调试会话未在 45 秒内就绪，请关闭 Grok 后重试";
...
mod tests {
    ...
    fn does_not_kill_debug_launch_in_progress() {
        ...
        let launching = Observation {
            processes: vec![record(31, 300).with_debug(Some(9337))],
            ..Observation::default()
        };
    ...
    fn attaches_ready_debug_session() {
        ...
        let ready = Observation {
            processes: vec![record(41, 400).with_debug(Some(9337))],
    ...
    fn electron_children_do_not_retrigger_takeover() {
        ...
        let children = Observation {
            processes: vec![
                record(61, 600).with_debug(Some(9337)),
                record(62, 601).with_debug(Some(9337)),
                record(63, 602),
            ],
            has_ready_debug_session: true,
            engine_alive: true,
    ...
    fn debug_port_wait_times_out_without_killing_and_rearms_after_exit() {
        ...
        let launching = vec![record(31, 300).with_debug(Some(9337))];
        assert_eq!(
        ...
        let with_child = vec![
            record(31, 300).with_debug(Some(9337)),
            record(32, 301).with_debug(Some(9337)),
        ];
        assert_eq!(
    ...
    fn encode_reprobe_cancels_stale_generation_and_only_attaches_live_session() {
        ...
        assert_eq!(
            confirm_launch_after_encode(
                ...
                &original,
                &Observation {
                    processes: vec![record(11, 100).with_debug(Some(9337))],
                    has_ready_debug_session: true,
                    ..Observation::default()
                }
            ),
    ...
    fn parses_remote_debugging_port() {
        assert_eq!(
            parse_remote_debugging_port(
                r#"Grok Bot.exe --remote-debugging-port=9337 --inspect-brk=127.0.0.1:9238"#
            ),
            Some(9337)
        );
        assert_eq!(
            parse_remote_debugging_port(r#""Grok Bot.exe" "--remote-debugging-port=9228""#),
            Some(9228)
        );
        assert_eq!(parse_remote_debugging_port("Grok Bot.exe"), None);
    }

    #[test]
    fn snapshot_of_missing_exe_is_empty() {
        let records = snapshot_executable_processes(r"C:\definitely-missing-grok\Grok Bot.exe")
    ...
    const GROK: &str = r"D:\grok_bot\Grok Bot\Grok Bot.exe";
    ...
    fn tracker_opens_only_new_processes_between_full_scans() {
        ...
        system.start(100, GROK, 2);
        system.start(101, GROK, 3);
        system.start(200, r"C:\Old\Grok Bot.exe", 4);
        let mut tracker = ProcessTracker::new();
        let start = Instant::now();

        let records = tracker.scan_with(&system, GROK, start).unwrap();
        assert_eq!(pids(&records), [(100, 2), (101, 3)]);
        assert_eq!(
            system.take_opened(),
            3,
            "only Grok Bot.exe-named processes are opened"
        );

        let tick = start + Duration::from_millis(500);
        assert_eq!(
            pids(&tracker.scan_with(&system, GROK, tick).unwrap()),
            [(100, 2), (101, 3)]
        );
        assert_eq!(
            system.take_opened(),
    ...
    fn tracker_rereads_missing_command_lines_and_recycled_pids() {
        ...
        system.edit(100, |process| {
            process.command_line = Some("Grok Bot.exe --remote-debugging-port=9229".to_string())
        });
        let tick = start + Duration::from_millis(500);
        let records = tracker.scan_with(&system, GROK, tick).unwrap();
        assert!(records[0].has_debug_args);
        assert_eq!(records[0].debug_port, Some(9229));

        system.exit(100);
        system.start(100, GROK, 7);
        let tick = start + Duration::from_millis(1000);
        let records = tracker.scan_with(&system, GROK, tick).unwrap();
    ...
    fn tracker_drops_an_exiting_process_like_a_snapshot_does() {
        ...
        let mut tracker = ProcessTracker::new();
        let start = Instant::now();
        tracker.scan_with(&system, GROK, start).unwrap();
    ...
    fn tracker_caches_inaccessible_processes_until_the_next_full_scan() {
        ...
        tracker
            .scan_with(&system, GROK, start + Duration::from_millis(500))
            .unwrap();
        assert_eq!(system.take_opened(), 1);
        tracker
            .scan_with(&system, GROK, start + Duration::from_millis(1000))
            .unwrap();
        assert_eq!(system.take_opened(), 0);

        system.exit(4);
        system.start(4, GROK, 9);
        let tick = start + Duration::from_millis(1500);
        assert!(tracker.scan_with(&system, GROK, tick).unwrap().is_empty());
        let full = start + FULL_SCAN_INTERVAL;
        assert_eq!(
            pids(&tracker.scan_with(&system, GROK, full).unwrap()),
            [(4, 9)]
        );
    }
