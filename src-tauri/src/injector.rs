...
const REMOVE_RENDERER_PAYLOAD: &str = r#"(() => {
  ...
  document.documentElement?.classList.remove(
    ...
    "grok-background-home-disabled", "grok-background-task-disabled", "grok-background-fit-tile"
  );
...
fn is_main_grok_target_url(value: &str) -> bool {
    let Ok(url) = Url::parse(value) else {
        return false;
    };
    match url.scheme() {
        // Grok Bot renderer is a local Electron page. Never attach to its VNC webview.
        "file" => {
            let path = url.path().to_ascii_lowercase().replace('\\', "/");
            let renderer = path.ends_with("/resources/app.asar/dist/renderer/index.html");
            let under_grok =
                path.contains("/grok_bot/grok%20bot/") || path.contains("/grok_bot/grok bot/");
            renderer && under_grok
        }
        _ => false,
    }
    ...
    Ok(targets
        ...
        .filter(|target| {
            target.target_type == "page"
                && is_main_grok_target_url(&target.url)
                && valid_id(&target.id)
...
mod tests {
    ...
    fn rejects_non_loopback_websocket_urls() {
        ...
        .is_ok());
        for value in [
            "ws://192.168.1.2:9337/devtools/page/page-1",
            "wss://127.0.0.1:9337/devtools/page/page-1",
            "ws://127.0.0.1:9338/devtools/page/page-1",
            "ws://user@127.0.0.1:9337/devtools/page/page-1",
        ] {
            assert!(validate_websocket_url(value, 9337, "page", Some("page-1")).is_err());
        }
    }

    #[test]
    fn injects_only_grok_app_pages() {
        assert!(is_main_grok_target_url(
            "file:///D:/grok_bot/Grok%20Bot/resources/app.asar/dist/renderer/index.html"
        ));
        assert!(!is_main_grok_target_url(
            "https://a41c0f769d25d59fac94-pod.example/vnc.html"
        ));
        assert!(!is_main_grok_target_url("http://localhost:3000/"));
        assert!(!is_main_grok_target_url(
            "file:///D:/grok_bot/Grok%20Bot/resources/app.asar/dist/renderer/other.html"
        ));
        assert!(!is_main_grok_target_url(
            "file:///D:/other/Grok%20Bot/resources/app.asar/dist/renderer/index.html"
        ));
    }
    ...
    #[test]
    #[ignore = "requires a live Grok CDP endpoint on port 9337"]
    fn reads_live_grok_browser_identity() {
        let identity = read_browser_identity(9337).expect("read live browser identity");
        assert!(valid_id(&identity));
    }

    #[test]
    #[ignore = "requires a live Grok CDP endpoint on port 9337"]
    fn reads_live_grok_with_reqwest() {
        let body = reqwest::blocking::get("http://127.0.0.1:9337/json/version")
    ...
    #[test]
    #[ignore = "requires a live Grok CDP endpoint on port 9337"]
    fn connects_to_live_grok_page_session() {
        let identity = read_browser_identity(9337).expect("read live browser identity");
        let target = list_targets(9337, &identity)
            .expect("list live targets")
            .into_iter()
            .next()
            .expect("at least one Grok page target");
        let session = CdpSession::open(&target, 9337).expect("open CDP page session");
        assert_eq!(
            session
                .evaluate("Boolean(document.documentElement)")
        ...
        let state = session
            .evaluate("({state:Boolean(window.__GROK_BACKGROUND_STUDIO__), styles:document.getElementById('grok-background-style')?.textContent?.length || 0, body:document.body?.innerText?.slice(0,80)})")
            .expect("verify injected background state");
        println!("live Grok injection state: {state:?}");
